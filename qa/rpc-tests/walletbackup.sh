...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars FLIRTCOIND and FLIRTCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

FLIRTCOIND=${FLIRTCOIND:-${1}/flirtcoind}
CLI=${FLIRTCOINCLI:-${1}/flirtcoin-cli}
...
B4ARGS="-datadir=$D4"
$FLIRTCOIND $FLIRTCOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$FLIRTCOIND $FLIRTCOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [ x$TXID = x ] ; then
      echoerr "node${1}: error sending ${3} flirt"
...
function StartThree {
  $FLIRTCOIND $FLIRTCOINDARGS $B1ARGS &
  B1PID=$!
  $FLIRTCOIND $FLIRTCOINDARGS $B2ARGS &
  B2PID=$!
  $FLIRTCOIND $FLIRTCOINDARGS $B3ARGS &
  B3PID=$!
}
