...
*Setup instructions for a gitian build of Flirtcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Flirtcoin
...
- [Building Flirtcoin](#building-flirtcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-flirtcoin-host/README.md).
...
Clone the git repositories for flirtcoin and gitian and then checkout the flirtcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/flirtcoin/flirtcoin
cd flirtcoin
...
In [doc/release-process.md](release-process.md) in the flirtcoin repository under 'Fetch and build inputs'.
...
To build Flirtcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the flirtcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/flirtcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/flirtcoin/flirtcoin
...
For example:
```bash
URL=https://github.com/laanwj/flirtcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit flirtcoin=${COMMIT} --url flirtcoin=${URL} ../flirtcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit flirtcoin=${COMMIT} --url flirtcoin=${URL} ../flirtcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit flirtcoin=${COMMIT} --url flirtcoin=${URL} ../flirtcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/flirtcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/flirtcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/flirtcoin-build.assert
```
...
[flirtcoin/gitian.sigs](https://github.com/flirtcoin/gitian.sigs/) repository, or if that's not possible create a pull
