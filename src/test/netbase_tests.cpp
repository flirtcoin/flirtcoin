// Copyright (c) 2012-2013 The Flirtcoin Core developers
...
{
    BOOST_CHECK(TestSplitHost("www.flirt.life", "www.flirt.life", -1));
    BOOST_CHECK(TestSplitHost("[www.flirt.life]", "www.flirt.life", -1));
    BOOST_CHECK(TestSplitHost("www.flirt.life:80", "www.flirt.life", 80));
    BOOST_CHECK(TestSplitHost("[www.flirt.life]:80", "www.flirt.life", 80));
...
{
    /* TODO - Replace with valid tor address
     * Other clients that wish to implement similar functionality, can use this test case:
     * 5wyqrzbvrdsumnok.onion == FD87:D87E:EB43:edb1:8e4:3588:e546:35ca.
     * The conversion is simply decoding the base32 onion address,
     * and storing the resulting 80 bits of data as low-order bits of an IPv6 address,
     * prefixed by fd87:d87e:eb43:. As this range is not routable, there should be no compatibility problems:
     * any unaware IPv6-capable code will immediately fail when trying to connect.
     *
