...
const int FLIRTCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString FLIRTCOIN_IPC_PREFIX("flirtcoin:");
const char* FLIRTCOIN_REQUEST_MIMETYPE = "application/flirtcoin-paymentrequest";
const char* FLIRTCOIN_PAYMENTACK_MIMETYPE = "application/flirtcoin-paymentack";
const char* FLIRTCOIN_PAYMENTACK_CONTENTTYPE = "application/flirtcoin-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(FLIRTCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // flirtcoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseFlirtcoinURI(arg, &r) && !r.address.isEmpty())
            {
                CFlirtcoinAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(FLIRTCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(FLIRTCOIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click flirtcoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start flirtcoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling flirtcoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on flirtcoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in flirtcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(FLIRTCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // flirtcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseFlirtcoinURI(s, &recipient))
            {
                CFlirtcoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
                ...
                emit message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Flirtcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CFlirtcoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Insecure payments to custom flirtcoin addresses are not supported
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            emit message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(FlirtcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", FLIRTCOIN_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, FLIRTCOIN_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", FLIRTCOIN_PAYMENTACK_MIMETYPE);
